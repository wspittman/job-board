...
suite("HTML", () => {
  {
    test("file run: converts markdown to HTML and writes output file", async () => {
    ...
    invalidFileNames.forEach((fileName) => {
      test(`file run: throws CommandError for invalid file name "${fileName}"`, async () => {
  ...
  {
    test("parseFrontmatter: parses all three fields and separates body", () => {
    ...
    invalidInputs.forEach(({ label, text }) => {
      test(`parseFrontmatter: returns undefined for ${label}`, () => {
        assert.equal(parseFrontmatter(text), undefined);
      });
    });

    test("parseFrontmatter: escapes HTML special characters in frontmatter values", () => {
  ...
  }
  {
    ...
    test("runBlog: converts frontmatter markdown to a full HTML page", async () => {
    ...
    invalidSlugs.forEach((slug) => {
      test(`runBlog: blog subcommand throws CommandError for invalid slug "${slug}"`, async () => {
        await assert.rejects(
    ...
    test("runBlog: throws CommandError when md file does not exist", async () => {
  ...
  }
  {
    ...
    test("updateBlogIndex: inserts card at top of blog list", async () => {
    ...
    test("updateBlogIndex: is idempotent when slug already present", async () => {
      touchedFiles.push(tmpPath);
  ...
  }
  {
    ...
    test("updateSitemap: inserts URL entry before closing urlset tag", async () => {
      touchedFiles.push(tmpPath);
    ...
    test("updateSitemap: is idempotent when slug already present", async () => {
      touchedFiles.push(tmpPath);
