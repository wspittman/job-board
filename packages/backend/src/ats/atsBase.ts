...
export abstract class ATSBase {
  ...
  abstract getBasicCompany(key: CompanyKey): Promise<Company>;
  abstract getCompany(key: CompanyKey): Promise<LLMContext<Company>>;
  abstract getJobs(key: CompanyKey, full?: boolean): Promise<LLMContext<Job>[]>;
  abstract getJob(key: CompanyKey, jobKey: JobKey): Promise<LLMContext<Job>>;
...
// TODO: Update to include jobId when available and differentiate between the two ids
