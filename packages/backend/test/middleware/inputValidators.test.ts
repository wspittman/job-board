  ...
  invalidCases.forEach((input) => {
    test(toTestName("Invalid input", { input }), () => {
      asyncLocalStorage.run({}, () => {
        ...
        assert.deepEqual(asyncLocalStorage.getStore() ?? {}, {});
      });
    });
  });
});
...
suite("useCompanyKey", () => {
  ...
  validCases.forEach((input) => {
    test(toTestName("Valid input", input), () => {
      ...
      assert.deepEqual(result, COMPANY_KEY);
    });
  });
...
suite("useCompanyKeys", () => {
  ...
  validCases.forEach((input) => {
    test(toTestName("Valid input", input), () => {
      ...
      assert.deepEqual(result, COMPANY_KEYS);
    });
  });
...
suite("useJobKey", () => {
  ...
  validCases.forEach((input) => {
    test(toTestName("Valid input", input), () => {
      ...
      assert.deepEqual(result, JOB_KEY);
    });
  });
...
suite("useFilters", () => {
  ...
  const invalidCases = [
    ...
    { workTimeBasis: 123 },
    { jobFamily: "invalid_value" },
    { jobFamily: 123 },
    { companyStage: "invalid_value" },
    { companyStage: 123 },
