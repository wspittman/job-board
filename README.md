...
Next-generation job board focused on the job seeker experience. The monorepo houses the Vite/Vanilla frontend, an Express 5 + TypeScript API, and a unified TypeScript CLI for safe API operations plus local evaluation/data workflows.
...
- **Workspaces**
  ...
  - [`packages/cli`](packages/cli/README.md): Unified TypeScript CLI for operational API actions and local evaluation/data workflows with safety defaults.
  - [`packages/ops`](packages/ops/README.md): Legacy operations CLI retained during migration.
  - [`packages/lab`](packages/lab/README.md): Legacy evaluation/data CLI retained during migration.
...
The unified CLI provides one entrypoint for API automation and local evaluation/data flows.
...
- End-to-end flows run only against the local profile.

See [`packages/cli/README.md`](packages/cli/README.md) for the full command list.
